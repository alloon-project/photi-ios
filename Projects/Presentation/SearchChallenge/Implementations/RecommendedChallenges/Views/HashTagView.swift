...
final class HashTagView: UIView {
  ...
  fileprivate var selectedHashTagRelay = CurrentValueSubject<String, Never>(Constants.total)

  var didSelectHashTag: AnyPublisher<String, Never> {
    selectedHashTagRelay
      .removeDuplicates()
      .eraseToAnyPublisher()
  }
...
private extension HashTagView {
  func bind() {
    allChip.tapGesturePublisher
      .sinkOnMain(with: self) { owner, _ in
        owner.selectedHashTagRelay.send(Constants.total)
...
extension HashTagView: UICollectionViewDelegate {
  func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
    ...
    selectedHashTagRelay.send(item)
