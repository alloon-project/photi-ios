...
final class FeedCommentTopView: UIView {
  ...
  var didTapLikeButton: AnyPublisher<Bool, Never> {
    likeButton.tapPublisher
      .map { [weak self] in self?.likeButton.isSelected ?? false }
      .eraseToAnyPublisher()
  }

  var didTapOptionButton: AnyPublisher<Bool, Never> {
    optionButton.tapPublisher
      .map { [weak self] in self?.optionButton.isSelected ?? false }
      .eraseToAnyPublisher()
  }
